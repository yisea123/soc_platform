 ...
 * V2.3 Fixing DMA Error reporting
 * V2.3 Fixing Dual role issue, session bit not going zero on disconnect event
 * SVN $Revision: 7515 $
...
static uint8_t MSS_USB_CIF_host_rx_errchk(mss_usb_ep_num_t ep_num)
{
    ...
    {
        status |= MSS_USB_EP_NAK_TOUT;
        /*Not clearing NAKTIMEOUT error here. Application may want to
        abort transfer. Clearing it here makes Scheduler keep trying the transfer*/
    }
