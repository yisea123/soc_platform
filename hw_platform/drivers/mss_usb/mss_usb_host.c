...
/*Use this at time of class allocation after device is enumerated
 Currently we support only one device.Hence one Class allocation, still an array
 is used so that future support for multiple devices would be easy.*/
...
/*Apart from this driver, Class drivers can also do a CEP transfers as per need.
  This variable is to differentiate between transfers started by this driver
  and CRP transfers started by Class driver.*/
...
)
{
    ...
    txep_ptr->num_usb_pkt = 1u;             /*Update with parameter*/
    txep_ptr->buf_addr = 0u;                /*update in Pipe_write*/
...
)
{
    ...
    rxep_ptr->num_usb_pkt = 1u;             /*Update with parameter*/
    rxep_ptr->buf_addr = 0u;                /*update in Pipe_write*/
...
)
{
       ...
       (tdev_ep_maxpktsz != 0) && (buf != 0) )
    {
        ...
        if((DMA_ENABLE == rxep_ptr->dma_enable) &&  (MSS_USB_XFR_BULK == rxep_ptr->xfr_type))
        {
...
)
{
    ...
    else
    {
        ...
        /*wIndex-LSB and MSB => other than String Desc, these values must be zero*/
        buf[4] = 0x00;
        buf[5] = 0x00;
    }
...
{
    MSS_USBH_CIF_bus_suspend(0u);   //suspendM enabled. RemoteWakeup not supported.
}
    ...
    */
    switch(g_enum_state)
    {
        ...
        case ENUM_WAIT_SET_TDEV_ADDR:
            if(g_cep_xfr_result)
            {
                if(MSS_USB_EP_XFR_SUCCESS == g_cep_xfr_result)
                {
                    ...
                    /*USB2.0-9.2.6.3:SetAddrDelay at least 2ms*/
        ...
        case ENUM_CHECK_TDEV_CLASS_SUPPORT:
            ...
            else if(cd_idx < 3u)                   /*Max No of classDriver is 3*/
            {
    ...
    else
    {
        if(MSS_USB_CIF_rx_ep_is_rxpktrdy((mss_usb_ep_num_t)ep_num))
        {
            ...
            else //dma disable
            {
                ...
                {
                    rxep_ptr->txn_count = 0u;//reset txn_count for next txn
...
)
{
    ...
    for(i=0;i<1;i++)    /*Max number of registered Class drivers supported is 1*/
    {
