    ...
    switch(g_msc_state)
    {
        ...
        case USBH_MSC_SCSI_READ_CAPACITY_DPHASE:
            if(g_usbh_msc_tx_event)
            {
                ...
                /*standard Read Capacity response size is 8bytes. We need this info
                  later so keep it in g_bot_readcap*/
                MSS_USBH_read_in_pipe(g_msd_tdev_addr,
                                      USBH_MSC_BULK_RX_PIPE,
                                      g_tdev_in_ep.num,
                                      g_tdev_in_ep.maxpktsz,
        ...
        case USBH_MSC_BOT_RETRY:
        {
            ...
            /*Found that the Sandisc devices are reporting NAKTIMEOUT error. This
            is mostly happening when moving from DATA phase to Status phase of
            the SCSI read/write command. At this stage restarting the status phase
            is able to get the device to respond properly. The NAKLIMIT is device
            specific. The WIndows hosts allow up to 5Sec before declaring NAKTIMOUT.
            MSS USB allows to wait up to ~4Sec. Hence waiting for another 1 Sec here
            hoping that the device is recovered by then.*/
            if(MSC_BOT_STATUS_WAITCOMPLETE == g_msc_bot_state)
            {
                if((crrent_mili - first_mili) >= 1000)
                {
                    first_mili = 0u;
                    crrent_mili = 0u;
    ...
    else
    {
        if(0 == status)
        {
            switch(g_msc_bot_state)
            {
                ...
                if(0 == g_scsi_command.dbuf_len)//zdl request
                {
